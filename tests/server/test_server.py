...
def run_server(model, host, port, num_devices):
    server = AsyncGenerationServer(model, host=host, port=port, num_devices=num_devices)
    server.run()
...
def server_model_fixture(request):
    ...
    process = multiprocessing.Process(target=run_server, args=(model, host, port, num_devices))
    process.start()
